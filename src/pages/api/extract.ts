...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    // Resolve pin.it shortlink if needed
    if (targetUrl.includes('pin.it')) {
      try {
        ...
        const redirectUrl = shortRes.headers.get('location');
        if (redirectUrl) {
          targetUrl = redirectUrl.startsWith('http') ? redirectUrl : `https://pinterest.com${redirectUrl}`;
        }
      } catch (e) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ error: 'Network error while connecting to Pinterest. Please check the link and try again.' }), { 
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    }

    if (!html || responseStatus !== 200) {
      return new Response(JSON.stringify({ error: `Pinterest returned status ${responseStatus}. The Pin may be private or deleted.` }), { 
        status: 404,
        headers: { 'Content-Type': 'application/json' }
      });
    }
    ...
    for (const match of relayMatches) {
      try {
        ...
        if (pinData) {
          ...
          // Annotations & Categories for Tags
    ...
    // Extract tags from title & description if tagsSet is small
